  ...
  const filteredArrays = posts.reduce((filtered, post) => {
    if (post.referenceName !== "wallpost") {
      ...
      const filteredComments = post.comments.filter(
        (comment) => comment.timeStamp.split(", ")[0] === todaysDate
      );
...
<div class="front-page">
            ...
            <a href="/profile/{user.artistName}" target="_blank">
              {#if user.profilePictureKey === "blank_profile.webp"}
                <div
                  ...
                  style="background-image: url('{imageSourcePrefix}{user.profilePictureKey}');"
                >
              ...
              {:else}
                <div
                  class="slide-content-wrapper"
                  style="background-image: url('{imageSourcePrefix}{user._id}/profile/{user.profilePictureKey}');"
                >
                  <div class="numbertext">
                    <p>{user.artistName}</p>
                  </div>
      ...
      {#each users as user, index}
        {#each user.discography as discography}
          {#if discography.isNewRelease === true || discography.length !== 0}
            <div
              class="mySlides-release fade-release"
              style="display: {index === 0 ? 'block' : 'none'};"
            >
              <h2>{discography.mainTitle}</h2>
              <div class="release-img-container">
                <h3>Release Date: {discography.releaseDate}</h3>
                <a href="/profile/{user.artistName}"
                  ><img
                    class="release-img"
                    src="{imageSourcePrefix}{user._id}/discography/{discography._id}"
                    alt=""
                  /></a
                >
              </div>
              <div class="release-content">
                ...
                <div class="release-service">
                  <a href={discography.mainUrl} target="_blank"
                    ><img
                      class="service-img"
                      src="{imageSourcePrefix}logos/{discography.selectedService}.svg"
                      alt=""
                    /></a
                  >
                </div>
  ...
  <div class="info-section">
    ...
    <div class="about">
      <h2>About Us</h2>
      <p>
        Tune Tower is a platform dedicated to supporting new musicians. Whether
        you're a singer, songwriter, producer, or instrumentalist, we provide
        you with the tools and resources to grow your music career.
      </p>
    </div>
    <div class="discover">
      <h2>Discover</h2>
      <p>
        Explore a wide range of musical genres, discover new artists, and find
        inspiration for your own music. Our curated collection of tracks and
        playlists will keep you engaged and motivated on your musical journey.
      </p>
    </div>
    ...
    <div class="connect">
      <h2>Connect</h2>
      <p>
        Connect with fellow musicians, collaborate on projects, and exchange
        ideas. Our community is filled with talented individuals who are
        passionate about music. Build your network and grow together.
      </p>
    </div>
    ...
    <div class="showcase">
      <h2>Showcase</h2>
      <p>
        Showcase your talent to the world. Create your artist profile, upload
        your music, and reach a global audience. Gain recognition, receive
        feedback, and take your music career to the next level.
      </p>
    </div>
  </div>
  ...
  <div class="top-5-posts">
    ...
    {#each topPosts as post}
      <div class="top-post">
        ...
        {:else}
          <a href="/forum/{post.referenceName}/{post.postTitle}#{post._id}"
            >Go to Post</a
          >
        {/if}
      </div>
