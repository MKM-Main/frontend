<script>
  ...
  export const updateUser = async () => {
    ...
    if (formData.get("profilePicture")) {
      const profilePictureRequest = fetch(
        `${PUBLIC_BASE_URL}api/admin/users/${artistId}/profile-picture`,
        {
          method: "PATCH",
          credentials: "include",
          headers: {
    ...
    if (Object.keys(userBody).length !== 0) {
      const updateUserRequest = fetch(
        `${PUBLIC_BASE_URL}api/admin/users/${artistName}`,
        {
          method: "PATCH",
          credentials: "include",
          headers: {
...
<div>
  <form on:submit|preventDefault={updateUser}>
    ...
    <label for="profilePicture" style={canUpload === false ? "color: red" : ""}
      >Profile Picture: {canUpload === false
    ...
    {#each tags as tag, idx}
      ...
      <input
        id="tag-{idx}"
        on:change={() => (tag.checked = !tag.checked)}
        checked={tag.checked}
        type="checkbox"
      />
    {/each}

    <button on:click={() => (showCurrentTags = !showCurrentTags)} type="button"
      >See current user tags</button
    >
    ...
    {#if showCurrentTags}
      <div class="current-tags">
        {#each userTags as currentTag, idx}
          ...
          <input
            id="currentTag-{idx}"
            on:change={() => (currentTag.checked = !currentTag.checked)}
            checked={currentTag.checked}
            type="checkbox"
          />
          <button type="submit" on:click={() => handleDeleteTag(idx)}
            >Delete</button
          >
        {/each}
      </div>
    ...
    <button
      class="btn-submit"
      class:btn-block-submit={canUpload === false}
